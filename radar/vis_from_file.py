...
boxes = []
nextFrame = None
pandata = pd.read_csv('radar_preGFM_C2.txt', names=['time', 'track', 'range', 'angle', 'trash', 'more trash', 'power'], header=0)
# for col in list(pandata.columns):
# 	pandata[col] = float(pandata[col])
...
#         time.sleep(.1)
        ...
        print(time.time() - startTime - delay, time.time()-startTime)
        pandata1 = pandata[(pandata['time'] > (time.time() - startTime - delay))&(pandata['time']<time.time()-startTime)]
        pandata1 = pandata1[pandata['range'] < 50.0]
        print(len(pandata1))
        return pandata1
